...
erDiagram
    ...
    TRANSACTIONS {
        ...
        enum fraud_status "APPROVED | REVIEW | BLOCKED, NULLABLE"
...
| `fraud_status` | `Enum(FraudStatus)` | `fraud_status` (enum nativo) | **Sim** | `default=None` | Decisão da antifraude: `APPROVED`, `REVIEW` ou `BLOCKED`. **Nullable** porque depósitos não são triados e linhas antigas não têm valor. Ortogonal ao `status` (ver seção 3) |
...
| `fraud_status` | `APPROVED`, `REVIEW`, `BLOCKED` | `transactions.fraud_status` | Decisão da antifraude, **nullable**. Adicionado via `CREATE TYPE` + `ADD COLUMN` (migração `a82ae7b338f7`). Ortogonal ao `transaction_status` |
...
No lado Python, cada enum é espelhado por uma classe `StrEnum` em `app/db/models.py` (`AccountStatus`, `TransactionType`, `TransactionStatus`, `FraudStatus`, `LedgerEntryType`), o que permite comparação direta com strings (`transaction.status == "COMPLETED"` funciona) mantendo tipagem estática no restante do código.
...
| `a82ae7b338f7` | *add fraud_status to transactions* | `CREATE TYPE fraud_status` + `ADD COLUMN transactions.fraud_status` (nullable) - habilita a antifraude. Diferente da migração de enum anterior, esta é **reversível** (o `downgrade` remove coluna e tipo) |

**Padrão observado:** cada migração corresponde a uma decisão de negócio rastreável (ver seção 9 do [docs/requisitos.md](requisitos.md)), nunca a uma alteração "solta" de schema. A ordem de aplicação (`down_revision` encadeado) foi validada rodando `alembic upgrade head` **do zero** em um banco descartável (e a de `fraud_status`, também com `downgrade`/re-`upgrade`), garantindo que a cadeia de migrações funciona de forma determinística em qualquer ambiente novo (CI, onboarding, produção).
